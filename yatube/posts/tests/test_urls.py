...
class PostURLTests(TestCase):
    ...
    def test_posts_edit_url_exists_for_author(self):
        ...
        response_auth_author = self.authorized_client_author.get(f'/posts/{self.post.id}/edit/')
        self.assertEqual(response_auth_author.status_code, 200)

        response_auth_user = self.authorized_client.get(f'/posts/{self.post.id}/edit/', follow=True)
