...
class PostViewTests(TestCase):
    ...
    def test_pages_uses_correct_template(self):
        ...
        templates_pages_names = {
            ...
            reverse('posts:group_list', kwargs={'slug': PostViewTests.group.slug}): 'posts/group_list.html',
            reverse('posts:profile', kwargs={'username': PostViewTests.author.username}): 'posts/profile.html',
            reverse('posts:post_detail', kwargs={'post_id': PostViewTests.post.id}): 'posts/post_detail.html',
    ...
    def test_profile_page_context(self):
        ...
        response = self.authorized_client.get(
            reverse('posts:profile', kwargs={'username': PostViewTests.author.username}))
    ...
    def test_post_detail_page_context(self):
        ...
        self.assertEqual(response.context.get('post').text, PostViewTests.post.text)
        self.assertEqual(response.context.get('post').author, PostViewTests.post.author)
        self.assertEqual(response.context.get('post').group, PostViewTests.post.group)


    def test_create_post_page_show_correct_context(self):
        """Шаблон posts/create_post.html сформирован с правильным контекстом."""
        templates = [
            reverse('posts:post_edit', kwargs={'post_id': PostViewTests.post.pk}),
...
class PostPaginatorViewTests(TestCase):
    ...
    def test_group_list_first_page_contains_ten_records(self):
        """Проверка: количество постов на первой странице posts/group_list.html равно 10."""
        response = self.client.get(
            reverse('posts:group_list', kwargs={'slug': PostPaginatorViewTests.group.slug}))
        self.assertEqual(len(response.context['page_obj']), POSTS_COUNT)
    

    def test_group_list_second_page_contains_three_records(self):
        """Проверка: количество постов на второй странице posts/group_list.html равно 3."""
        response = self.client.get(
            reverse('posts:group_list', kwargs={'slug': PostPaginatorViewTests.group.slug}) + '?page=2')
        posts_on_second_page = Post.objects.count() % POSTS_COUNT
        self.assertEqual(len(response.context['page_obj']), posts_on_second_page)


    def test_profile_first_page_contains_ten_records(self):
        """Проверка: количество постов на первой странице posts/profile.html равно 10."""
        response = self.client.get(
            reverse('posts:profile', kwargs={'username': PostPaginatorViewTests.author.username}))
        self.assertEqual(len(response.context['page_obj']), POSTS_COUNT)
    

    def test_profile_second_page_contains_three_records(self):
        """Проверка: количество постов на второй странице posts/profile.html равно 3."""
        response = self.client.get(
            reverse('posts:profile', kwargs={'username': PostPaginatorViewTests.author.username}) + '?page=2')
        posts_on_second_page = Post.objects.count() % POSTS_COUNT
        self.assertEqual(len(response.context['page_obj']), posts_on_second_page)